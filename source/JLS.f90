...
checknumemax=0
quick=0
specification="IERsubmission"

!! Loop through estimated parameters to compute the gradient
...
1000 format(I3)
open(unit=323,file=pathoutput//adjustl(trim(ctag))//'sigma'//charac//'.txt',position='rewind')
endif
...
if (runsolution==1) then
    open(8987,file=pathoutput//adjustl(trim(ctag))//'theta_out_'//specification//'.txt', position='rewind')
