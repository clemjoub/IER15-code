...
open(120,file=emaxfile,position='rewind')
write(120,*) Emax
close(120)
open(130,file=afile,position='rewind')
write(130,*) statemat_a
close(130)
open(140,file=Bhfile,position='rewind')
write(140,*) statemat_Bh
close(140)
open(145,file=Bwfile,position='rewind')
...
open(150,file=thetafile,position='rewind')
write(150,*) theta_out
close(150)
open(160,file=rsqfile,position='rewind')
write(160,*) rsq_out
close(160)
...
!open(180,file=d2file,position='rewind')
!write(180,*) d_out(2,:,:,:)
!close(180)
!open(190,file=d3file,position='rewind')
!write(190,*) d_out(3,:,:,:)
!close(190)
...
open(210,file=d4file,position='rewind')
!write(210,*) d_out(4,:,:,:)
close(210)
open(220,file=d5file,position='rewind')
!write(220,*) d_out(5,:,:,:)
close(220)
open(230,file=d6file,position='rewind')
!write(230,*) d_out(6,:,:,:)
close(230)
...
open(240,file=d7file,position='rewind')
!write(240,*) d_out(7,:,:,:)
close(240)
open(250,file=d8file,position='rewind')
!write(250,*) d_out(8,:,:,:)
close(250)
open(260,file=d9file,position='rewind')
!write(260,*) d_out(9,:,:,:)
close(260)
...
!open(270,file=savingsfile,position='rewind')
!write(270,*) avsav_out
!close(270)
!open(280,file=incomefile,position='rewind')
!write(280,*) avy_out
!close(280)
open(290,file=regressorsfile,position='rewind')
write(290,*) reg
close(290)
...
open(340,file=std_errorsfile,position='rewind')
...
open(700,file=simulatedMPfile,position='rewind')
open(600,file=valuefunctionfile,position='rewind')
open(500,file=onehouseholdfile,position='rewind')
open(5000,file=simulationfile,position='rewind')	
...
CLONESLOOP:		do i=1,nclones
				...
				YEARLOOP:	do yr=first_year(h),last_year(h),1
								write(5000,'(I5,I5,I10, f20.10,I5,I5,I5,4f15.5,I4,4f15.5,8I3,I6,I3)') h,i,folio(h), simv(h,1,yr-1979), simcohort(h,i,yr-1979),sexsampled(h), simage(h,i,yr-1979),&
